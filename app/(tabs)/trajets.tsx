...
export default function TrajetsScreen() {
  ...
  const appliquerFiltres = () => {
    ...
    if (filterDate.trim()) {
      result = result.filter(t =>
        t.date_depart.startsWith(filterDate)
      );
    }
    if (filterPrixMax.trim()) {
      result = result.filter(t =>
        t.prix <= parseFloat(filterPrixMax)
      );
    }
    if (filterPlaces.trim()) {
      result = result.filter(t =>
        t.places_dispo >= parseInt(filterPlaces)
      );
    }
  ...
  const filtresActifs = filterDepart || filterArrivee || filterDate || filterPrixMax || filterPlaces;
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Filter button */}
      <TouchableOpacity
        style={[styles.filterBtn, filtresActifs ? styles.filterBtnActive : null]}
        onPress={() => setFilterVisible(true)}>
        <Text style={[styles.filterBtnText, filtresActifs ? styles.filterBtnTextActive : null]}>
      ...
      <FlatList
        ...
        style={{ marginTop: 10 }}
        ListEmptyComponent={
          <Text style={styles.empty}>Aucun trajet ne correspond aux filtres.</Text>
        }
        renderItem={({ item }) => (
        ...
        <View style={styles.fond}>
          <ScrollView>
            <View style={styles.popup}>
              ...
              <TouchableOpacity style={styles.button} onPress={appliquerFiltres}>
                <Text style={styles.buttonText}>Appliquer</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.boutonFermer} onPress={reinitialiserFiltres}>
                <Text style={styles.buttonText}>Réinitialiser</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.boutonAnnuler} onPress={() => setFilterVisible(false)}>
      ...
      <Modal visible={trajetChoisi != null} transparent animationType="slide">
        <View style={styles.fond}>
          <View style={styles.popup}>
            ...
            <Text style={styles.infoLine}>📅 Date : {trajetChoisi?.date_depart}</Text>
            <Text style={styles.infoLine}>💰 Prix : {trajetChoisi?.prix} TND / place</Text>
            <Text style={styles.infoLine}>💺 Places disponibles : {trajetChoisi?.places_dispo}</Text>
            <Text style={styles.infoLine}>📝 Description : {trajetChoisi?.description ?? 'Aucune'}</Text>

            <TouchableOpacity
              style={[styles.boutonReserver, trajetChoisi?.places_dispo === 0 && styles.boutonReserverDisabled]}
              disabled={trajetChoisi?.places_dispo === 0}
              onPress={allerAuFormulaire}>
              <Text style={styles.buttonText}>
                {trajetChoisi?.places_dispo === 0 ? 'Complet' : 'Réserver une place'}
              </Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  toast: {
    ...
    backgroundColor: '#1D9E75', padding: 14, borderRadius: 12, zIndex: 999
  },
  ...
  filterBtn: {
    ...
    borderWidth: 1, borderColor: '#4f46e5', alignItems: 'center'
  },
